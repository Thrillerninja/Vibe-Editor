...
// Responsive git-style history graph now owns its own history state and exposes
// an imperative `addCommit` method via ref. It also handles revert confirmation
// internally and notifies the parent with `onRevertConfirmed(entry)` when a revert
// is accepted.
export default forwardRef(function HistoryGraph({ initialHistory = [], className = 'history-graph', onRevertConfirmed = () => {} }, ref) {
    const n = initialHistory.length; // used as baseline for effect dependency
    ...
    const [headIndex, setHeadIndex] = useState(history.length ? history.length - 1 : null);
    const [pendingRevert, setPendingRevert] = useState(null);

    // Compute nodes, edges, and SVG dimensions for rendering from local `history`
    const { nodes, edges, laneYs, contentWidth, svgHeight } = useMemo(() => {
        ...
        history.forEach((commit, i) => {
            ...
            if (commit.parentId == null) {
                x = xOffset;
                nodes.push({ i, x, y, lane, ts: commit.ts, text: commit.text, title: commit.title, data: commit.data });
            } else {
    ...
    // Imperative handle: expose addCommit to parent components via ref
    useImperativeHandle(ref, () => ({
        addCommit: (data, title) => {
            const id = history.length;
            const ts = Date.now();

            setHistory((h) => {
                if (h.length === 0) {
                    const newCommit = { id, ts, data, title, parentId: null, branchId: 0 };
                    setHeadIndex(0);
                    return [newCommit];
                }

                let branchId = h[headIndex]?.branchId ?? 0;
                let parentId = h[headIndex]?.id ?? null;
        ...
        getHistory: () => history,
        getHeadIndex: () => headIndex,
    }), [history, headIndex]);

    // Revert interactions handled internally
    const handleNodeRevertRequest = (index) => {
    ...
    const confirmRevert = () => {
        ...
        // Notify parent to update the sentences (or other state)
        try {
            onRevertConfirmed(entry);
        } finally {
            setHeadIndex(index);
    ...
    return (
      ...
      <div className={`w-full h-full ${className}`} style={{ position: 'relative', boxSizing: 'border-box'}}>
              ...
              return (
                ...
                >
                  ...
                  <circle
                    ...
                    style={{ outline: 'none' }}
                    onClick={() => handleNodeRevertRequest(node.i)}
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleNodeRevertRequest(node.i); }}
                    onBlur={hideTooltip}
                    tabIndex={0}
                    role="button"
        ...
        {/* Revert confirmation modal (internal) */}
        {pendingRevert != null && (() => {
