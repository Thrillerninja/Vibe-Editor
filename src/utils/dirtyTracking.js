...
export function addSentenceToHierarchy(sentences, newSentence, insertIndex) {
    ...
    // Check if there's an existing placeholder chain we can reuse
    // This prevents creating multiple "New content (pending AI)" nodes
    console.log(`${LOG_PREFIX.PARSER} Checking for existing placeholder chain`);

    const { v4: uuidv4 } = require('uuid');
    const maxLevel = hierarchyMeta.maxLevel || 2;

    // Try to find existing placeholder nodes at each level and reuse them
